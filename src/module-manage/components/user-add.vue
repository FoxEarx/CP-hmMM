<template>
  <div class="add-form">
    <el-dialog :title="text" :visible.sync="dialogFormVisible" @close="onClose">
      <el-form
        :rules="ruleInline"
        ref="dataForm"
        :model="formBase"
...
export default {
  name: "usersAdd",
  props: ["text", "formBase", "ruleInline"],
  data() {
    return {
  ...
  methods: {
    ...
    async getPermissionsList() {
      ...
      this.PermissionGroupsList = res.data.list;
    },
  },
  // 挂载结束
...
const fun = (arr) => { arr.forEach(item => { if (item.parent) { item.children =
item.parent return } this.fun(item.children) }) } this.fun(arr)
data.forEach(item => { if (item.children) { aa(item.children) } else if
(item.parent) { 直接处理 } }) funcction aa (item) { item.forEach(item => { if
(item.parent) { 处理 return } aa(item.children) }) }
